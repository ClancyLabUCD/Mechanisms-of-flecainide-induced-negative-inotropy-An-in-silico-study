...
void f( double dt0, double t, double * y, int p, double * ydot, double * currents , SimState1 *S){
    ...
    const double pi = 3.141592653589793; // pi = 3.1415926535897932384626433832795;
    ...
    //  Cell geometry
    const double cellLength = 100;     //  cell length [um]
    const double cellRadius = 10.25;   //  cell radius [um]
    ...
    const double Cao = 1.8;  //  Extracellular Ca  [mM]1.8
    ...
    // //
    const double GNa = 25;
    const double GNaB = 0.597e-3;    //  [mS/uF] 0.897e-3
    const double IbarNaK = 1.0 * 1.8;// 1.90719;     //  [uA/uF]
    const double KmNaip = 11;         //  [mM]11
    ...
    const double gkp = 2 * 0.001;
    
    //  Cl current parameters
    const double GClCa = 0.5 * 0.109625;   //  [mS/uF]
    const double GClB = 1 * 9e-3;        //  [mS/uF]
    const double KdClCa = 100e-3;    //  [mM]
    
    //  I_Ca parameters
    
    
    double IC50_flecCaL = 27.1e-6;//53e-6;//10e-6;//20e-6;
    double factor_FlecCaL = 1/(1+(drug_onCaL/IC50_flecCaL)); // about 0.83333 if IC50 = 10uM and flec = 2 uM
    
    const double pNa = 0.50 * 1.5e-8 * factor_FlecCaL;       //  [cm/sec]
    const double pCa = 0.50 * 5.4e-4 * factor_FlecCaL;       //  [cm/sec]
    const double pK = 0.50 * 2.7e-7 * factor_FlecCaL;        //  [cm/sec]
    const double Q10CaL = 1.8;
    
    // //  Ca transport parameters
    const double IbarNCX = 1.0 * 4.5;      //  [uA/uF]5.5 before - 9 in rabbit
    ...
    const double IbarSLCaP = 0.0673; //  IbarSLCaP FEI changed [uA/uF](2.2 umol/L cytosol/sec) jeff 0.093 [uA/uF]
    const double KmPCa = 0.5e-3;     //  [mM]
    const double GCaB = 5.513e-4;    //  [uA/uF] 3
    const double Q10SLCaP = 2.35;    //  [none]
    
    //  SR flux parameters
    const double Q10SRCaP = 2.6;          //  [none]
    const double Vmax_SRCaP = 1.0 * 5.3114e-3;  //  [mM/msec] (286 umol/L cytosol/sec)
    const double Kmf = 0.246e-3;          //  [mM] default
    // Kmf = 0.175e-3;          //  [mM]
    const double Kmr = 1.7;               //  [mM]L cytosol
    const double hillSRCaP = 1.787;       //  [mM]
    const double ks = 25;                 //  [1/ms]
    const double koCa = 10;               //  [mM^-2 1/ms]   // default 10   modified 20
    ...
    const double koff_tncl = 19.6e-3;    //  [1/ms]
    ...
    const double I_nak_junc = ( 1. * Fjunc * IbarNaK * fnak * Ko
                               / ( 1 + pow( (KmNaip / y[31]) , 4 ) )
                               / ( Ko + KmKo ) );
    const double I_nak_sl = ( 1. * Fsl * IbarNaK * fnak * Ko
                             / ( 1 + pow( ( KmNaip / y[32] ), 4 ) )
                             / ( Ko + KmKo ) );
...
//        double dI = (bi*O  - I*(ai));
    ...
    if( markov_iks==0 ) {
        const double gks_junc=1.*0.0035;
        const double gks_sl=1.*0.0035; // FRA
        const double xsss = 1. / (1+exp(-(y[38] + 3.8)/14.25)); //  fitting Fra
        ...
        const double gks_junc=0.0065;
        const double gks_sl=0.0065; // FRA
        const double alpha=3.98e-4*exp(3.61e-1*y[38]*FoRT);
        const double beta=5.74e-5*exp(-9.23e-2*y[38]*FoRT);
        const double gamma1 = 3.41e-3*exp(8.68e-1*y[38]*FoRT);
        const double delta=1.2e-3*exp(-3.3e-1*y[38]*FoRT);
        const double teta=6.47e-3;
        const double eta=1.25e-2*exp(-4.81e-1*y[38]*FoRT);
        const double psi=6.33e-3*exp(1.27*y[38]*FoRT);
        const double omega=4.91e-3*exp(-6.79e-1*y[38]*FoRT);
        
        ydot[41]=-4*alpha*y[41]+beta*y[42];
        ydot[42]=4*alpha*y[41]-(beta+gamma1+3*alpha)*y[42]+2*beta*y[43];
        ydot[43]=3*alpha*y[42]-(2*beta+2*gamma1+2*alpha)*y[43]+3*beta*y[44];
        ydot[44]=2*alpha*y[43]-(3*beta+3*gamma1+alpha)*y[44]+4*beta*y[45];
        ydot[45]=1*alpha*y[43]-(4*beta+4*gamma1)*y[45]+delta*y[49];
        ydot[46]=gamma1*y[42]-(delta+3*alpha)*y[46]+beta*y[47];
        ydot[47]=2*gamma1*y[43]+3*alpha*y[46]-(delta+beta+2*alpha+gamma1)*y[47]+2*beta*y[48]+2*delta*y[50];
        ydot[48]=3*gamma1*y[44]+2*alpha*y[47]-(delta+2*beta+1*alpha+2*gamma1)*y[48]+3*beta*y[49]+2*delta*y[51];
        ydot[49]=4*gamma1*y[45]+1*alpha*y[48]-(delta+3*beta+0*alpha+3*gamma1)*y[49]+2*delta*y[52];
        ydot[50]=1*gamma1*y[47]-(2*delta+2*alpha)*y[50]+beta*y[51];
        ydot[51]=2*gamma1*y[48]+2*alpha*y[50]-(2*delta+beta+1*alpha+gamma1)*y[51]+2*beta*y[52]+3*delta*y[53];
        ydot[52]=3*gamma1*y[49]+1*alpha*y[51]-(2*delta+2*beta+2*gamma1)*y[52]+3*delta*y[54];
        ydot[53]=1*gamma1*y[51]-(3*delta+1*alpha)*y[53]+beta*y[54];
        ydot[54]=2*gamma1*y[52]+1*alpha*y[53]-(3*delta+1*beta+1*gamma1)*y[54]+4*delta*y[55];
        ydot[55]=1*gamma1*y[54]-(4*delta+teta)*y[55]+eta*y[56];
        const double O2=1-(y[41]+y[42]+y[43]+y[44]+y[45]+y[46]+y[48]+y[47]+y[49]+y[50]+y[51]+y[52]+y[53]+y[54]+y[55]+y[56]);
        ydot[56]=1*teta*y[55]-(eta+psi)*y[56]+omega*O2;
        const double I_ks_junc = Fjunc*gks_junc*(y[56]+O2)*(y[38]-eks);
        const double I_ks_sl = Fsl*gks_sl*(y[56]+O2)*(y[38]-eks);
        I_ks = I_ks_junc+I_ks_sl;
    }
    // end
    ...
    if( epi==1 ) {
        GtoSlow=1.0*0.13*0.12 * factor_FlecIto; // epi
        GtoFast=1.0*0.13*0.88 * factor_FlecIto; // epi0.88
    } else {
        GtoSlow=0.13*0.3*0.964 * factor_FlecIto; // endo
        GtoFast=0.13*0.3*0.036 * factor_FlecIto; // endo
        // end
    }
    ...
    const double tauytos = 800./(1+exp((y[38]+60.0)/10))+30;
    ...
    const double I_ki =1.* 0.35*sqrt(Ko/5.4)*kiss*(y[38]-ek);
    ...
    const double fss = 1./(1+exp((y[38]+35)/9))+0.6/(1+exp((50-y[38])/20));
    
    // //  I_Ca: L-type Calcium Current
    const double dss = 1./(1+exp(-(y[38]+5)/6.0));
    const double taud = dss*(1-exp(-(y[38]+5)/6.0))/(0.035*(y[38]+5));
    //  fss = 1/(1+exp((y[38]+35.06)/3.6))+0.6/(1+exp((50-y[38])/20));
    const double tauf = 1./(0.0197*exp( - pow( (0.0337*(y[38]+14.5)), 2) )+0.02);
    ydot[3] = (dss-y[3])/taud;
    ydot[4] = (fss-y[4])/tauf;
    ydot[5] = 1.7*y[35]*(1-y[5])-11.9e-3*y[5]; //  fCa_junc   koff!!!!!!!!
    ydot[6] = 1.7*y[36]*(1-y[6])-11.9e-3*y[6]; //  fCa_sl
    double fcaCaMSL = 0.1/(1+(0.01/y[36]));
    double fcaCaj= 0.1/(1+(0.01/y[35]));
    fcaCaMSL=0;
    fcaCaj= 0;
    ...
    const double I_Ca_junc = (Fjunc_CaL*ibarca_j*y[3]*y[4]*((1-y[5])+fcaCaj) * pow( Q10CaL, Qpow ) ) * 0.45 * 1;
    const double I_Ca_sl = (Fsl_CaL*ibarca_sl*y[3]*y[4]*((1-y[6])+fcaCaMSL) * pow( Q10CaL, Qpow ) ) * 0.45 * 1;
    const double I_Ca = I_Ca_junc+I_Ca_sl;
    const double I_CaK = (ibark*y[3]*y[4]*(Fjunc_CaL*(fcaCaj+(1-y[5]))+Fsl_CaL*(fcaCaMSL+(1-y[6]))) * pow( Q10CaL, Qpow ) ) * 0.45 * 1;
    const double I_CaNa_junc = (Fjunc_CaL*ibarna_j*y[3]*y[4]*((1-y[5])+fcaCaj) * pow( Q10CaL, Qpow ) ) * 0.45 * 1;
    const double I_CaNa_sl = (Fsl_CaL*ibarna_sl*y[3]*y[4]*((1-y[6])+fcaCaMSL) * pow( Q10CaL, Qpow ) ) * .45 * 1;
    ...
    const double s3_junc = KmCai * Nao * Nao * Nao * ( 1 + pow( ( y[31] / KmNai ), 3) ) + KmNao * KmNao * KmNao * y[35] * ( 1 + y[35] / KmCai ) + KmCao * y[31] * y[31] * y[31] + y[31] * y[31] * y[31] * Cao + Nao * Nao * Nao * y[35];
    ...
    const double J_SRleak = 5.348e-6 * ( y[30] - y[35] );           //    [mM/ms]
    ...
    const double J_CaB_cytosol = ydot[18] + ydot[19] + ydot[20] + ydot[21] + ydot[22] + ydot[23] + ydot[24];
    ...
    double I_app, rate, factor, V_hold, V_test, V_clamp, R_clamp;
    ...
    switch( protocol ) {
        ...
        case 1: // 'pace1',        //  pace w/ current injection at rate 'rate'
            // rate =  1e-3;
            rate = 1. / cycleLength; // 1e-3;  // SVP 11/12/09 in 4Hz?
        ...
        case 2: // 'pacenew',        //  pace w/ current injection at rate 'rate'
            if ( ( t >= 0 ) && ( t <= 5 ) ) {
                I_app = 9.5; }
            else {
                if ( ( t >= 310 ) && ( t <= 315 ) ) {
        ...
        case 3: // 'pace4',
            ...
            if ( (t > 1000) & (t < 2000) & (mod( t + 900. , 1. / rate) <= 5 ) ) {
                I_app = 10; }
            else if ( (t > 2000) & (mod( t + 900. , 1. / rate * 10. ) <= 5 ) ) {
                I_app = 10; }
            else {
                I_app = 0; }
            // end
        ...
        case 7: // 'recovery',
            ...
            // tdelay=[10 1 5 80];
            if ( (mod( t , 1. / rate ) <= 1000 || mod( t - 1000 + 1. / rate - tdelay[ (int)floor(t*rate) ] , 1. / rate ) <= 1000) ){
                V_clamp = -20; }
            else {
                V_clamp = -80; }
            // end
    ...
    const double I_Cl_tot = I_ClCa+I_Clbk;                        //  [uA/uF]
