...
void f( double dt0, double t, double * y, int p, double * ydot, double * currents , SimState1 *S){
    ...
    double drug_onNa = 0;//S->drug;
    
    double drug_onKr = 0;//S->drug;
    
    double drug_onRyR = 0;//S->drug;
    
    double drug_onIto = 0;//S->drug;
    ...
    //  rtoss = 1/(1+exp((y[38]+33.5)/10));
    const double tauxtos = 9./(1+exp((y[38]+3.0)/15))+0.5;
    const double tauytos = 800./(1+exp((y[38]+60.0)/10))+30;
    //  taurtos = 2.8e3/(1+exp((y[38]+60.0)/10))+220; // Fei changed here!! time-dependent gating variable
    ...
    // tauxtof = 3.5*exp(-((y[38]+3)/30)^2)+1.5;
    const double tauytof = 85 * exp( (- pow( (y[38]+40), 2) / 220 ) ) + 7;
    // tauytof = 20.0/(1+exp((y[38]+33.5)/10))+20.0;
    ...
    //     ydot[41] = 0;
    // else
    // ydot[40] = -I_Na*Cmem/(Vmyo*Frdy);
    // ydot[41] = -I_Ca_sl*Cmem/(Vjunc*2*Frdy)*Vjunc/Vmyo;
    // end
